...
String.prototype.toSlug = function () {
    return this.toLowerCase().trim()
        .replace(/(à|á|ạ|ả|ã|â|ầ|ấ|ậ|ẩ|ẫ|ă|ằ|ắ|ặ|ẳ|ẵ)/g, 'a') // Xóa dấu tiếng việt
        ...
        .replace(/(\s+)/g, '-') // Đổi khoảng trắng thành ký tự gạch ngang
        .replace(/^\-+/g, '') // Nếu có nhiều ký tự gạch ngang "-" thì đổi thành một gạch ngang
        .replace(/\-+/g, '-') // Nếu có nhiều ký tự gạch ngang "-" thì đổi thành một gạch ngang
        .replace(/\-+$/g, '');

}
$('input[name=name]').on('change', function () {
    $('input[name=slug]').val($(this).val().toSlug());
})
$('.ckfinder, #cat_thumbnail').on('click', function () {
    CKFinder.popup({
        selectActionFunction: function (fileUrl) {
            $('.ckfinder, #cat_thumbnail').val(fileUrl).trigger('change');
        },
        customConfig: '',
        skin: 'bootstrap'
    });
}).on('change', function () {
    $('.preview').removeClass('hidden').attr('src', $(this).val());
});

$('.delete').click(function () {
    if (window.confirm('Bạn chuẩn bị xóa mục này.\nBấm "OK" để xóa, "Hủy" để hủy')) {
        return true;
    }
    return false;
});
...
$(".add-more").click(function(){
    var html = $(".copy").html();
    $(".append-add-more").append(html);
});
$('.modal').on('click', '.product-img', function () {
    var sel = $(this);
    CKFinder.popup({
        selectActionFunction: function (fileUrl) {
            sel.val(fileUrl);
